...
//
// Decoding represents values the same way as the deprecated `getAny()`: numbers become `Double`,
// `null` and `undefined` become `NSNull`, and `undefined` object properties are dropped. Unlike
// `getAny()`, it never drops data silently: a bigint becomes `Int64` (throwing when it doesn't fit),
// and a function or symbol anywhere in the value throws `TypeError`, since neither has a native form.

extension JavaScriptValue {
  // MARK: - Decoding

  /// Decodes a JavaScript value into a free-form native value: `Bool`, `Double`, `String`, `Int64`
  /// (from a bigint), `[Any]`, `[String: Any]` or `NSNull`.
  ///
  ...
  /// cast to `type`. For an optional `type`, `null` and `undefined` decode as `nil`.
  ///
  ...
  public static func decodeAny<T>(
    ...
    if value.isNull() || value.isUndefined(), let optionalType = T.self as? any FreeFormOptional.Type {
      // The cast can't fail: `optionalType` is `T.self`.
      return optionalType.none as! T
    }
  ...
  public static func encodeAny(_ value: Any, in runtime: borrowing JavaScriptRuntime) throws -> JavaScriptValue {
    ...
    // checked first. The number and boolean checks compare the exact type rather than casting, since
    // an `NSNumber` casts to both `Bool` and `Double` and has to be told apart below.
    if let string = value as? String {
      return JavaScriptValue(copy runtime, string)
    }
    ...
    if valueType == Int.self {
      return .number(Double(value as! Int))
    }
...
/// Lets the free-form conversions work with an `Optional` whose wrapped type isn't known statically:
/// creating `nil` for an optional target type, and unwrapping an optional found inside `Any`.
